...
echo "Excluding this script file ($script_name)"
...
find "$directory" -depth -name "*leaf*" -not -name "$script_name" -print0 | while IFS= read -r -d '' file; do
...
# 处理不包含 "leaf" 在文件名中的文件
find "$directory" -type f -not -name "*leaf*" -not -name "$script_name" -print0 | while IFS= read -r -d '' file; do
    replace_content "$file"
done
